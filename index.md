...
### 8. [용어 정리](#8)
### 9. [개발 작업](#9)
### 10. [1주차 작업 결과](files/w01/test.md)
...
|속성|영문명칭|설명
|---|---|---|
|무기 아이템|weaponItem|짧은검,장작도끼,단창,검,도끼,창,군용검,전쟁도끼,장창,장검,양손도끼,사냥활,긴활,석궁,나무화살,돌화살,나무볼트,돌볼트,횃불,삼지창|
...
## 1. A, D (프로그래밍 - 캐릭터 스탯 시스템)
- ```파라미터 - 캐릭터```를 토대로 캐릭터 스탯 시스템을 구축한다. 
- 유니티상에서 C# 유니티 스크립트를 생성한다. 이후 생성된 유니티 스크립트안에 있는 클래스에서 ```파라미터 - 캐릭터```테이블 안에 있는 파라미터들을 전부 public 변수로 선언한다.
- 스킬, 숙련도같은 경우, public 형태로 레벨만 변수로 취급하고 ```Update```함수를 이용하여 스킬, 숙련도 시스템을 구축해둔다.
## 2. C, F (그래픽 - 캐릭터)
...
|캐릭터 명|모델 타입|설명|
|---|---|---|
|주인공|인간형|플레이어가 조종하는 캐릭터의 모델|
|주인공(토르형태)|인간형|플레이어가 조종하는 캐릭터의 모델|
|마을 주민 남성|인간형|툰드라 북부에 사는 평범한 마을 주민|
|마을 주민 여성|인간형|툰드라 북부에 사는 평범한 마을 주민|
|마을 주민 아이|인간형|툰드라 북부에 사는 평범한 마을 주민|
|마을 주민 노인 남성|인간형|툰드라 북부에 사는 평범한 마을 주민|
|마을 주민 노인 여성|인간형|툰드라 북부에 사는 평범한 마을 주민|
|청년 바이킹 전사|인간형|바이킹 전사. 가죽 갑옷을 입고 있다.|
|중년 바이킹 전사|인간형|바이킹 전사. 사슬 갑옷을 입고 있다.|
|노년 바이킹 전사|인간형|바이킹 전사. 사슬 갑옷과 바이킹 투구를 입고 있다.|
|해골|인간형|강령술에 의해 되살아난 망자. 해골이 무기를 들고 싸우는 모습.|
|드로거|인간형|북유럽 신화에 등장하는 언데드.|
|트롤|2족 짐승형|멍청하지만 체구가 크고 힘이 매우 강한 괴물|
|화염거인|거인형|거대한 체구에 영원히 불타는 몸을 가진 거인. 키는 대략 5미터 정도.|
|거인|거인형|거대한 체구에 원시인같은 모습을 한 거인. 키는 대략 3미터 정도.|
|늑대|4족 짐승형|추운 툰드라에서 사는 지능적이고 똑똑한 늑대들.|
|늑대인간|2족 짐승형|늑대와 인간이 섞인 반인반수 형태의 괴물.|
|시 서펜트|바다뱀 반인반수형|바다뱀과 인간이 섞인듯한 형태의 괴물. 하반신은 바다뱀, 상반신은 인간 그리고 머리는 바다뱀이다.|


## 3. B, E (프로그래밍 - AI)
- ```A, D```에서 만든 캐릭터 스탯 시스템과 ```C, F```에서 만든 캐릭터 모델을 토대로 캐릭터 AI 시스템을 구축한다.
### 3-1. 목적 변수
- 우선, 모든 NPC들을 위한 ```목적 변수```를 만든다. 목적 변수의 목적은 AI가 현재 무엇을 해야할지 결정하는 것이다.
- 그러기 위해선, AI가 가질 수 있는 모든 목적의 종류를 만들어야 한다. ```Enum```을 사용하여 목적의 종류를 만들고, 목적 변수의 자료형 또한 ```Enum```으로 선언한 변수를 이용하여 자료형으로 이용한다.
...
|---|---|
|대기|설명 생략|
|이동|해당 지점으로 이동한다. 목적 지점 변수: ```목표 지점```이 필요하다.|
|일|일하는중. 일을 하기 위한 목적 오브젝트 변수: ```노동이 가능한 상호작용 오브젝트```이 필요하다(예를 들면 장작을 패는곳 이라던지).|
|식사|밥을 먹거나, 물을 마신다. 목적 지점 변수: ```식사 장소```가 필요하다.
|대화|어떤 캐릭터와 대화한다. 대화할 목적 오브젝트 변수: ```캐릭터```가 필요하다.
|취침|잔다. 목적 오브젝트 변수: ```취침 장소```가 필요하다.
|기상|일어난다. 
|전투|해당 캐릭터 그룹과 전투를 하는 중이다. 목적 오브젝트 변수 ```캐릭터```가 필요하다. 이 경우, 단체전이 일어날 가능성을 고려하여 배열과 for문을 이용하여 구축하도록 한다.
...
|4|분기 발생 - 가족이 있는가? 5번 : |
|5|집으로 향한다(자신의집)|
|6|가족과 저녁식사를 한다. - 식사(자신의집)|오후 7시|
|7|취침(자신의집)|오후8시|
|8|주점으로 간다. 이동(주점)|오후 7시
|9|주점에서 저녁을 먹는다. 식사(주점)|오후 8시|
...
## 3. G (프로그래밍 -아이템)
- 해당 주차에서 만든 아이템 시스템과 아이템을 AI가 사용할 수 있도록 한다.
